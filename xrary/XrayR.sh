...
install() {
    bash <(curl -Ls https://pub-eed78fedcfb6470ea94589a3771b4e0f.r2.dev/xray/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://pub-eed78fedcfb6470ea94589a3771b4e0f.r2.dev/xray/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://pub-eed78fedcfb6470ea94589a3771b4e0f.r2.dev/xray/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
